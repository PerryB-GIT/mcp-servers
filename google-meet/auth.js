...
async function authenticate() {
  ...
  const oAuth2Client = new google.auth.OAuth2(
    client_id,
    client_secret,
    'http://localhost:3337/oauth2callback'
  );
  ...
  const server = createServer(async (req, res) => {
    try {
      const url = new URL(req.url, 'http://localhost:3337');
      if (url.pathname === '/oauth2callback') {
        ...
        if (code) {
            ...
            <html>
              <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #fff;">
                <div style="text-align: center;">
                  <h1 style="color: #6366f1;">✅ Google Meet Authentication Successful!</h1>
          ...
          console.log('🎉 Google Meet authentication complete!\n');
